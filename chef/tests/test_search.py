...
from chef.tests import ChefTestCase, mockSearch, test_chef_api

class SearchTestCase(ChefTestCase):
    def test_search_all(self):
        s = Search('node', self.api)
    ...
    def test_search_query(self):
        s = Search('node', self.api, 'role:test_1')
    ...
    def test_rows(self):
        s = Search('node', self.api, rows=1)
        self.assertEqual(len(s), 1)
        self.assertGreaterEqual(s.total, 3)

    def test_start(self):
        s = Search('node', self.api, start=1)
        self.assertEqual(len(s), s.total-1)
        self.assertGreaterEqual(s.total, 3)

    def test_slice(self):
        s = Search('node', self.api)[1:2]
        self.assertEqual(len(s), 1)
        self.assertGreaterEqual(s.total, 3)
...
class MockSearchTestCase(ChefTestCase):
    @mockSearch({
        ('node', '*:*'): [Node('fake_1', test_chef_api(), skip_load=True).to_dict()]
    })
    def test_single_node(self, MockSearch):
        import chef.search
        s = chef.search.Search('node', self.api)
        self.assertEqual(len(s), 1)
